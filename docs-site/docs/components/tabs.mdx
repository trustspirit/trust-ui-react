...
For a standalone segmented control without `Tabs.Content` panels, use the dedicated [`<SegmentedControl>`](./segmented-control.mdx) primitive instead.
...
- Active-tab indicator uses `var(--tui-duration-base)` + `var(--tui-ease-out)` for the slide animation — it's a layout-shift transition (the indicator's position and width genuinely change), which is the one case where trust-ui uses `transform` for anything beyond a hover response.
