...
The footer pins to the bottom of the viewport (not the scroll container) with `position: sticky` + `bottom: 0`. Its background uses the opaque `--tui-paper` so it doesn't bleed scrolled content through it.
...
By default, the footer uses a subtle 1px top border (`--tui-rule`) to separate from content. Set `elevated` to add `--tui-shadow-overlay` on top of that — the background stays the same opaque `--tui-paper`, only the shadow is added — useful when the content above scrolls **through** the footer (e.g., long forms).
...
| Variant | Style |
|---|---|
| Default | `--tui-rule` 1px top border + opaque `--tui-paper` background |
| `elevated` | Same background and border, plus `--tui-shadow-overlay` |
...
<PropsTable props={[
  { name: 'elevated', type: 'boolean', default: 'false', description: 'Add --tui-shadow-overlay on top of the default border, for visual lift' },
