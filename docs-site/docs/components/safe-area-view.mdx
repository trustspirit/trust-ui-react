...
function FullscreenScreen() {
  return (
    <SafeAreaView style={{ height: '100vh', background: 'var(--tui-paper)' }}>
      <Header />
      <Content />
    </SafeAreaView>
  );
...
<Dialog open={open} onClose={close} mobileVariant="fullscreen">
  {/* mobileVariant="fullscreen" already applies safe-area padding via CSS — you don't need to add SafeAreaView yourself */}
</Dialog>
```
...
```tsx
<header style={{ background: 'var(--tui-accent)', paddingTop: 0 }}>
  <SafeAreaView edges={['top']}>
    <div style={{ padding: 'var(--tui-gap-4)' }}>
      <h1>Settings</h1>
    </div>
  </SafeAreaView>
</header>
