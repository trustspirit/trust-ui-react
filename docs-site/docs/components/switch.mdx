...
- Switch knob slides with `var(--tui-duration-base)` + `var(--tui-ease-out)` — v2 has a single easing curve, so on and off use the same motion rather than mirrored in/out curves.
