...
- All hover effects (a `background-color`/`border-color` change, per variant) are guarded by `@media (hover: hover)` so they don't get "stuck" after a tap on touch devices.
