...
Tables don't fit small viewports gracefully. `mobileLayout` lets you pick the strategy:
...
```tsx
<Table mobileLayout="stacked" columns={cols} data={rows} />
```
...
<PropsTable props={[
  ...
  { name: 'zebra', type: 'boolean', default: 'false', description: 'Alternate row backgrounds with --tui-bg-muted' },
  { name: 'mobileLayout', type: "'scroll' | 'stacked'", default: "'scroll'", description: 'On narrow viewports: horizontal scroll or stacked-card per row' },
