...
- The calendar popup is an opaque `--tui-sheet` surface with a `--tui-shadow-overlay` shadow — there is no translucency.
...
<PropsTable props={[
  ...
  { name: 'mobileVariant', type: "'modal' | 'native'", default: "'modal'", description: 'Mobile rendering: themed fullscreen dialog or native OS picker' },
